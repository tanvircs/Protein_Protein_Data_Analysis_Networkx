...
closeness_centrality_dic = nx.closeness_centrality(G)    #Calculate the Closeness centrality of the network which will return a dictionary 
with open("Closeness_Centrality_Output.txt","w") as f:    #Create a text file name Closeness_Centrality_Output 
    f.write("\t\t\t\t\t\t\t\t\t************************************\t\t\tCloseness Centrality Output\t\t\t************************************"+"\n")
     #Write a header title 
    for k,v in closeness_centrality_dic.items():   #Iterate into the dictionary
